<?php
// Installer for SonikBB Forum
// Save this file as installer.php and run it in your browser
...
if (isset($_POST['submit'])) {
    ...
    //$path      = trim($_POST['path']); // Uncomment if you allow custom path
    $site_title  = trim($_POST['site_title']);
    $website     = trim($_POST['website']);
    $websitedesc = trim($_POST['websitedesc']);
    ...
    if (empty($errors)) {
        ...
        if (!$conn) {
            ...
            if (!mysqli_query($conn, $sql)) {
                ...
                // Read SQL dump (make sure your SQL dump file uses utf8mb4 settings)
                ...
                if (empty($errors)) {
                    ...
                    $insert_admin_sql = "INSERT INTO `users` (`username`, `password`, `type`, `last_login`, `email`, `created`)
                                         VALUES ('$admin_username', '$admin_password', 2, NOW(), 'admin@example.com', NOW())";
                    if (!mysqli_query($conn, $insert_admin_sql)) {
                        ...
                        // If send_data checkbox is checked, collect and send installation data
                        ...
                        // Prepare configuration file content
                        $config_content = "<?php\n\n";
                        ...
                        // Encode configuration content for preview
...
<div class="container">
    ...
    <form method="post">
        ...
        <input type="text" name="db_host" id="db_host" value="<?php echo htmlspecialchars($db_host); ?>" required>

        <label for="db_username">Database Username:</label>
        <input type="text" name="db_username" id="db_username" value="<?php echo htmlspecialchars($db_username); ?>" required>

        <label for="db_password">Database Password:</label>
        <input type="password" name="db_password" id="db_password" required>

        <label for="db_name">Database Name:</label>
        <input type="text" name="db_name" id="db_name" value="<?php echo htmlspecialchars($db_name); ?>" required>

        <label for="site_title">Site Title:</label>
        <input type="text" name="site_title" id="site_title" value="<?php echo htmlspecialchars($site_title); ?>" required>

        <label for="website">Website URL:</label>
        <input type="text" name="website" id="website" value="<?php echo htmlspecialchars($website); ?>" required>
        
        <label for="websitedesc">Website Description:</label>
        <input type="text" name="websitedesc" id="websitedesc" value="<?php echo htmlspecialchars($websitedesc); ?>" required>
        ...
        <input type="checkbox" name="send_data" id="send_data" checked>
